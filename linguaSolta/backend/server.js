...
app.get('/', (_req, res) => res.json({ status: 'ok', message: 'API Site Lingua Solta online.' }));
...
app.use((error, _req, res, _next) => {
  if (error instanceof multer.MulterError) {
    return res.status(400).json({ erro: error.code === 'LIMIT_FILE_SIZE' ? 'Arquivo muito grande.' : error.message });
  }
